...
public class UtilisateurDao implements IUtilisateurDao{
    ...
    public  void add(Utilisateur o) {
        try {
        	...
        	PreparedStatement stmt = connection.prepareStatement("INSERT INTO users(nom,prenom,email,password,active) VALUES (?,?,?,ENCRYPT(?,'driss'),1)");
	...
	public Utilisateur findByEmailAndPassword(String email, String password) {
		Utilisateur utilisateur = null;
		 try {	
			 	PreparedStatement stmt = connection.prepareStatement("SELECT * FROM users WHERE email=? AND `password`=ENCRYPT(?,'driss') ");
			 	stmt.setString(1, email);
